...
export class HotelAgent {
  ...
  static async getLocationCoordinates(destination) {
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(destination)}&key=${HOTEL_CONFIG.GOOGLE_PLACES_API_KEY}`
      );
    ...
    try {
      ...
      if (data.results) {
        return data.results.slice(0, HOTEL_CONFIG.MAX_RESULTS).map(hotel => ({
          ...
          photo: hotel.photos && hotel.photos[0] 
  ...
  static calculateDistance(coord1, coord2) {
    ...
    const distance = Math.sqrt(
      Math.pow(coord1.lat - coord2.lat, 2) + Math.pow(coord1.lng - coord2.lng, 2)
