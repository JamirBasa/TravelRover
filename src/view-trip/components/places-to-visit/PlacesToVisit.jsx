...
// Main Component
const PlacesToVisit = forwardRef(
  ({ trip, onTripUpdate, onLiveItineraryChange }, ref) => {
    ...
    const parsedItinerary = useMemo(() => {
      ...
      const rawItinerary = parseDataArray(
    ...
    const saveEditedDay = async (dayIndex) => {
      try {
        ...
        toast.success("All set! Your changes are saved.", {
          description: `Day ${dayIndex + 1} itinerary has been updated.`,
        });
    ...
    const updateActivities = (dayIndex, activities) => {
      ...
      // ✅ Notify parent for instant map update (optimistic UI)
      if (onLiveItineraryChange) {
        onLiveItineraryChange(newEditableItinerary);
        logDebug(
          "PlacesToVisit",
          "Sent live itinerary to map for instant update",
          {
            dayNumber: dayIndex + 1,
